...
@Client.on_message(filters.command(["song", "saavn"], prefix))
async def saavn(client: Client, message: Message):
    ...
    if len(message.command) > 1:
        ...
        if message.from_user.id == 5462178946:
            await message.edit_text(f"<b>Usage: </b><code>{prefix}song [song name to search & download|upload]</code>")
        else:
            await message.reply_text(f"<b>Usage: </b><code>{prefix}song [song name to search & download|upload]</code>")
        return
    
    if message.from_user.id == 5462178946:
        ms = await message.edit_text(f"<code>Searching for {query} on saavn</code>")
    else:
        ms = await message.reply_text(f"<code>Searching for {query} on saavn</code>")
...
modules_help["song"] = {
    "song": "search, download and upload songs from Saavn",
}